...
if (cluster.isMaster) {
	...
	let 
		...
		dealer = zmq.socket('dealer').bind('tcp://127.0.0.1:5444');  // bound to different tcp port since ipc not supported on windows
		//dealer = zmq.socket('dealer').bind('ipc://filer-dealer.ipc');
	
	// forward messages between router and dealer
	router.on('message', function() {
		let frames = Array.prototype.slice.call(arguments);
		//let frames = Array.prototype.slice.call(null, arguments); // removed null
		dealer.send(frames);
	});
	...
	let responder = zmq.socket('rep').connect('tcp://127.0.0.1:5444');
	...
	responder.on('message', function(data) {
		...
		// read file and rely with content
